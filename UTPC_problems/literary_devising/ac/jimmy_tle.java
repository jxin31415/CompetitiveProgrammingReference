...
public class jimmy_tle {
    ...
    static class Graph {
        ...
        private boolean dfs(int u, boolean[] visited, int target) {
            if (u == target)
                return true;
            ...
            for(int each: adj.get(u)) {
                if (!visited[each]) {
                    answer = answer || dfs(each, visited, target);
                }
            }
        ...
        // cycle from u to v, else false.
        private boolean isCyclic(int u, int v)
        {
            boolean[] visited = new boolean[V];
            boolean[] visited2 = new boolean[V];
     
            return dfs(u, visited, v) && dfs(v, visited2, u);
        }
    }
